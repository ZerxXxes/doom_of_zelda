...
describe('loadLevel', () => {
  it('parses width, height, gridSize, theme, name', () => {
    const lvl = loadLevel(fixture as LevelJson);
  ...
  it('throws on row length mismatch', () => {
    ...
    expect(() => loadLevel(bad as LevelJson)).toThrow(/row length/i);
  });
  ...
  it('throws on unknown legend char', () => {
    ...
    expect(() => loadLevel(bad as LevelJson)).toThrow(/unknown.*char/i);
  });

  it('throws when row count does not match height', () => {
    const bad = { ...fixture, tiles: ['#####', '#####'] };
    expect(() => loadLevel(bad as LevelJson)).toThrow(/row count/i);
  });
});
